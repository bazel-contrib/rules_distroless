...
load(":oci_base.bzl", "read_base", "stanza_field")
...
#
# `members`, when set, is every package the dependency set installs for this
# architecture, which onto a base is less than the whole closure: a
# dependency that is not one of them is the base's.
def package_deps_for_architecture(packages, package, architecture, mergedusr = False, members = None):
    return [
        "@" + util.package_repo_name(dep_key, mergedusr = mergedusr) + "//:data"
        for dep_key in package["depends_on"]
        if packages[dep_key]["architecture"] in [architecture, "all"] and (members == None or dep_key in members)
    ]

def base_status(stanzas, architecture, replaced):
    """The dpkg status of a base, less the packages a dependency set replaces."""
    kept = [
        stanza
        for stanza in stanzas
        if not (
            stanza_field(stanza, "Package") in replaced and
            stanza_field(stanza, "Architecture") in [architecture, "all"]
        )
    ]
    return "".join([stanza + "\n\n" for stanza in kept])
...
""".format(name = package_name, arch = architecture, v1 = recorded_version, v2 = source_version))
            ...
            format_vars = {
                ...
                "deps": package_deps_for_architecture(
                    packages,
                    package,
                    architecture,
                    mergedusr = rctx.attr.mergedusr,
                    members = {
                        short_key + "=" + version: True
                        for (short_key, version) in dependency_set["sets"][architecture].items()
                    } if rctx.attr.bases else None,
                ),
                "urls": package["urls"],
                "name": package["name"],
    ...
    # Onto a base, the image's dpkg status is the base's and the set's
    # together, so what the base installed is still known to be.
    base_status_attr = ""
    if rctx.attr.bases:
        statuses = {}
        for label in rctx.attr.bases:
            base = read_base(rctx, label)
            if base.architecture not in dependency_set["sets"]:
                continue
            statuses["//:linux_%s" % base.architecture] = "base_status_%s" % base.architecture
            rctx.file("base_status_%s" % base.architecture, base_status(
                base.stanzas,
                base.architecture,
                architectures_to_package_names[base.architecture],
            ))
        base_status_attr = "\n    base = select(%s)," % starlark_codegen_utils.to_dict_attr(statuses)
    ...
    rctx.file("BUILD.bazel", _ROOT_BUILD_TMPL.format(
        ...
        base_status = base_status_attr,
    ))

translate_dependency_set = repository_rule(
    implementation = _translate_dependency_set_impl,
    attrs = {
        "bases": attr.label_list(doc = "INTERNAL: the images the dependency set is installed onto (apt.install's `bases`)."),
